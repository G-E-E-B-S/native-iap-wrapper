...
static bool js_PluginIAPJS_IAP_consume(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 2) {
        ...
        SE_PRECONDITION2(ok, false, "js_PluginIAPJS_IAP_consume : Error processing arguments");
#ifdef IS_ANDROID
        sdkbox::IAP::consume(arg0, arg1);
#endif
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 1);
...
SE_BIND_FUNC(js_PluginIAPJS_IAP_consume)
...
static bool js_PluginIAPJS_IAP_queryPurchases(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    if (argc == 0) {
...
SE_BIND_FUNC(js_PluginIAPJS_IAP_queryPurchases)

static bool js_PluginIAPJS_IAP_queryUnconsumedPurchases(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    if (argc == 0) {
...
SE_BIND_FUNC(js_PluginIAPJS_IAP_queryUnconsumedPurchases)
...
///
static bool js_PluginIAPJS_IAP_initPlayPass(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 1) {
        std::string arg0;
        ok &= seval_to_std_string(args[0], &arg0);
        SE_PRECONDITION2(ok, false, "js_PluginIAPJS_IAP_initPlayPass : Error processing arguments");
#ifdef IS_ANDROID
        sdkbox::IAP::initPlayPass(arg0);
#endif
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 1);
...
SE_BIND_FUNC(js_PluginIAPJS_IAP_initPlayPass)
///
...
static bool js_PluginIAPJS_IAP_isEnabled(se::State& s)
{
    const auto& args = s.args();
    size_t argc = args.size();
    if (argc == 0) {
        auto ok = boolean_to_seval(true, &s.rval());
        SE_PRECONDITION2(ok, false, "js_PluginIAPJS_IAP_isEnabled : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_PluginIAPJS_IAP_isEnabled)
...
bool js_register_PluginIAPJS_IAP(se::Object* obj)
{
    ...
    cls->defineStaticFunction("consume", _SE(js_PluginIAPJS_IAP_consume));
    ...
    cls->defineStaticFunction("queryPurchases", _SE(js_PluginIAPJS_IAP_queryPurchases));
    cls->defineStaticFunction("queryUnconsumedPurchases", _SE(js_PluginIAPJS_IAP_queryUnconsumedPurchases));
    ...
    cls->defineStaticFunction("initPlayPass", _SE(js_PluginIAPJS_IAP_initPlayPass));
    ...
    cls->defineStaticFunction("isEnabled", _SE(js_PluginIAPJS_IAP_isEnabled));
